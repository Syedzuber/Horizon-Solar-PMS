...
| `payment_request` | `PaymentRequest` — `pending_approval \| approved \| on_hold \| rejected \| confirmed` | **INSTRUMENTED-PENDING — the subject exists, NO call site writes it yet.** Added by O1 (migration `0091`, 22 Sep 2026): the constant, the `SUBJECT_TYPE_CHOICES` entry and the registry/resolver entries. The call sites land in O4 (approve / hold / reject) and O5 (confirm). **Until then a missing `payment_request` row means "not instrumented", exactly as it did when this row sat in the second table.** Move this note to a list of write sites when O4 lands |
