...
**Version 1.17 — 26 Sep 2026.** One §12 row, added by session B3b: a change request is named by who raised it — SCM's is a BOQ change request, a PM's or coordinator's a design change request, a screen showing both says change request — and no screen says a request with the PM waits for the Design Head. Two figures change: the change-request rejection rate counts only requests that reached the Head, and requests by stage gains "Raised after release". Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D53.

**Version 1.16 — 26 Sep 2026.** One §12 row, added by session B3a: change requests are findable — the PM queue lists every change request on the viewer's sites with what happened to it, the design review queue lists every request waiting for the Design Head across tenders (Head authority only, capped at 25 with the rest counted), and the tender dashboard gains the "corrected" button. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D52, including the complete list of labels B3b's wording sweep must fix.

**Version 1.15 — 25 Sep 2026.** One §12 row, added by session B2b. It adds the screens and notifications for the SCM change-request route: the Design Head's "corrected" button on the QC review banner, the PM queue's SCM change-request section and its renamed nav entry, time-with-the-Head ageing, a "PM (change request)" Pending-at label, one list of who may triage, and the D-12 notification audiences. B2a's hard dependency (the button) is closed. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D51, including the deferred Head pull of a stranded request.

**Version 1.14 — 25 Sep 2026.** One §12 row, added by session B2a: an SCM-raised design change request is routed through the site's PM (forward, PM reject, SCM withdrawal), the Design Head can record a request as corrected in the BOQ, and a request with the PM blocks a group lock and a QC verdict. The B1 row's hard dependency is removed: B2a replaced both templates' catch-all branches. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D50, including a new hard dependency: B2b must add the "corrected" button.

**Version 1.13 — 25 Sep 2026.** One §12 row, added by session B1: the schema for routing an SCM-raised design change request through the PM (four new verdict values, `origin`, the PM-stage, withdrawal and correction fields, migration `0102`). Nothing writes the new values yet. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D49, including a hard dependency: B2 must not deploy without B3.

**Version 1.12 — 24 Sep 2026.** Eight §12 rows, a §5 notifications note and a §13 correction, added by the O6 session: every payment figure — the payments queue, the Finance dashboard and the CEO dashboard — comes from one count, read through the order; `project_overview` card 4b lists orders, not payments, and no longer pays anything; the payment detail page is a redirect to the order; the stand-alone raise and confirm views and `PaymentRequest`'s five legacy columns are gone (migration `0101`, which refuses rather than discards); only Finance and CEO may hold the payment-approver flag; and `record_transition(notify=False)` suppresses the one signal-driven send. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §28.

**Version 1.11 — 24 Sep 2026.** Five §12 rows and a §13 correction, added by the O5 session: Finance works vendor payments from one queue with a tab per project type, taken from the order and never from the anchor; the person who approved a request and the person who raised it cannot mark it paid; a payment reference is mandatory; and `invoice_paid` is gone until O7, replaced by four in-app notices. §13's `payment_request` row gains its sixth write site — confirming now records `approved -> confirmed` — and §6's sentence about the missing `confirmed` row is corrected to match. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §27.

**Version 1.10 — 24 Sep 2026.** Four §12 rows and a §13 rewrite, added by the O4 session: a raised vendor payment now waits for an approver, who may approve it, hold it with a reason for SCM to answer, or — only once it has been held — reject it finally; nobody decides on a request they raised themselves. §13's `payment_request` row moves from INSTRUMENTED-PENDING to a list of five write sites, and names the one that is still missing (`confirm_payment_request`, O5's); `PaymentRequestHold` joins the uninstrumented table, and §6's sentence about what a missing row means is corrected to match. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §O4-1 to §O4-4.

**Version 1.9 — 18 Sep 2026.** One §12 row, added by the typed-date session: a date a person types must fall between 01 Jan 2020 and today + 5 years, checked in the views and forms by `forms.check_typed_date()`, never by a model constraint. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` G6–G10.

**Version 1.8 — 17 Sep 2026.** One §12 row, added by the D18 session: lifting a Design Hold taken from an Arka review status restores that status (R1–R4). Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D18 and §D45–§D47.

**Version 1.7 — 17 Sep 2026.** One §12 row, added by session 3.1c-ii: design change requests notify in-app AND by email (N-a to N-e), the first design-module sends that name email. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D44.

**Version 1.6 — 16 Sep 2026.** One §12 row, added by session 3.1c-i: the design-change window for the PM and SCM, and the reversal of Part 4.6's choice to remove a draft-group site from its group at raise — it now leaves at acceptance. Nothing else in the file changed. Its open ends are `EXECUTION_MODULE_DEFERRED.md` §D43.

**Version 1.5 — 16 Sep 2026.** The correction pass over what v1.4 left standing. It fixes the four claims recorded as `EXECUTION_MODULE_DEFERRED.md` §D36: §5's *"mirrors leave both halves of a progress fraction"* (R-20's progress half counts them), §12's Site Engineer count in the 31 Aug `enable_cascade_scheduling` row (*"Nine of the 22"* was **wrong when it was written** — the 31 Aug seed gave the Site Engineer 11 of 22 — corrected in place, with a dated note for the 1 Sep seed change), §13's *"this table is the whole answer"* (three status-bearing models are in neither table), and §5's deploy-gate sentence, which prompt 3.1b-3 (`683a0ea`) made false by satisfying the gate it named. It also restates the deployment state after `683a0ea`, and carries a sweep of every claim in this file about what is deployed, held or pending — nine examined, two false. Recorded as `EXECUTION_MODULE_DEFERRED.md` §D42.

**THE RULE, so this stops recurring: EVERY EDIT TO THIS FILE GETS A VERSION.** v1.3 was edited in place through 3 Sep without a bump, and that is on record below as the problem this rule avoids. A reader holding a copy must be able to tell from the version string alone whether it says what this one says; two different texts under one version number make that impossible, and the three replace-this-copy notices below only work because the string moves when the content's truth moves.
...
**The payment's `project` is a display anchor, not its scope (O2d, 23 Sep 2026).** A
`PaymentRequest` is still raised against a `VendorOrder`, and the order is what says how
much and for what. `project` merely decides which site the payment is LISTED under, is
nullable, and is NULL on a payment against an order that names no site. The rule in this
decision — extend `PaymentRequest`, never replace it, never add a second invoice model —
is unaffected; what O2d settles is that no reader may read `project` as scope. See §12
(23 Sep).

**`VendorOrderDocument` is not an invoice model (O1, 22 Sep 2026).** It stores FILES — a PO, a PI, an invoice, anything else — against a `VendorOrder`. A row of `doc_type='invoice'` carries the vendor's invoice number and amount so an order's invoiced total can be summed, and nothing more: it has no status, no payer and no payment. **Payments remain `PaymentRequest`**, which O1 extended with `vendor_order`, an approval lifecycle and `approved_by` / `approved_at` / `decision_reason` rather than replacing. The rule above stands.
...
**R-20 · The question a number answers decides whether mirrors are in it. Two questions, two helpers.** *(Established by prompt 1.3b, 30 Aug 2026 as one rule. **Split into two by prompt 1.6, 1 Sep 2026**, after 1.5 made the single-rule version false.)*

A **mirror** (`Task.is_mirror`) has its status derived from another object — a `DesignAssignment`, an accepted delivery quantity, a COD record — and **no human may write it**, which since prompt B22 is enforced rather than merely stated: `_apply_task_status_change()` refuses a mirror outright, above the transition table (R-18).

**THE RULE, both halves:**

| Question | Mirrors | Helper | Where |
|---|---|---|---|
| **"How much of this SITE is done"** | **IN** | `site_progress_tasks_q(prefix='')` | Progress bars, percentages, per-project completion pairs |
| **"How much work does this PERSON or TEAM owe"** | **OUT** | `human_owned_tasks_q(prefix='')`, or `is_human_owned(task)` for a prefetched list | Overdue, pending, blocked, workload, department rollups, the EOD digest, the user status report |
...
**THE SINGLE-RULE VERSION WAS TRUE FROM 1.3b TO 1.5, AND THIS IS WHY IT STOPPED BEING.**

1.3b's rule was *"a task metric excludes mirrors"*, full stop, applied at **32 call sites in three files**. 1.5 then removed the exclusion from `project_overview`'s per-phase `phase_data_json` loop, because a phase holding only mirrors rendered **"0/0 done" above an empty bar while the card header three lines up printed "4 tasks"** — one phase, two numbers, neither wrong on its own terms and the pair unreadable. **0/4 tells a PM four deliveries are outstanding; 0/0 tells them nothing.** That change was right.
...
**WHY A `Q` AND NOT A FILTERED QUERYSET**, unchanged from 1.3b: three consumers have no queryset to filter — two conditional aggregates (`Count(..., filter=…)` on the CEO project cards and the SE dashboard, where filtering the base would change what the *other* annotations on the same join count) and one prefetched Python list (`project_overview`'s per-phase progress). The workload `Q` is always **positive**: a negated `Q` across the multi-valued `phases__tasks` relation takes Django's `exclude()` subquery path instead of a SQL `FILTER`, which would change the join fan-out the CEO card counts deliberately share.

**WHY `site_progress_tasks_q()` IS A NO-OP AND EXISTS ANYWAY.** It returns an empty `Q()`, matches every task, and compiles to `FILTER (WHERE True)` — semantically identical to passing no filter. It is not there to filter. It is there so that a counter which **deliberately** counts mirrors is distinguishable from one that **forgot** to exclude them; without it those two are byte-identical, and that ambiguity is the entire defect 1.6 exists to close. It accepts and ignores `prefix`, so swapping one helper for the other at a call site is a one-word diff.

**Not a custom manager**, and that is a deliberate re-affirmation. There are **no custom managers anywhere in this codebase**; soft delete depends on it, because `Task.objects` meaning "everything" is what forces each queryset to state its own predicate. Two predicates now exist and neither has a manager, for the same reason.
...
**Enforced by** `projects/tests_mirror_metrics.py` (two guards) and `projects/tests_progress_vs_workload.py` (the behaviour, on a real activated site).

**The structural guard enforces BOTH categories, from one comparison.** `ProgressWorkloadDifferentialSweepTests` renders each dashboard **twice over the same fixture — once with 0 mirrors, once with 6** — and diffs the two context walks key by key. The invariant is a property rather than a magic value:

> **A WORKLOAD number does not move when mirrors are added. A PROGRESS number does.**

The set of keys whose integer *changed* is therefore the set of counters that count mirrors, **derived from the running code rather than asserted about it**, and one assertion (`changed == PROGRESS_KEYS`) bites in both directions: a workload counter that forgets the exclusion **appears** undeclared, and a progress counter that gains one **disappears** while still declared. Both failures name the context key. Proven to bite both ways by breaking one site in each direction.

`ALLOWED_BROKEN_COUNTS` — 1.3b's allow-list of exceptions to a rule that is no longer true — **became `PROGRESS_KEYS`, a positive declaration of the second category.** Eight entries today: two `phase_data_json` keys, four `projects_with_progress` keys, two `project_cards` keys, each with the reason.

**Two properties of that guard, stated rather than implied.**
- **Ratios are absent from `PROGRESS_KEYS` on purpose.** `internal_percent` and `phase_data_json[…].pct` *are* progress numbers, but adding mirrors proportionally does not move them — 0/1 and 0/7 are both 0%. The set means "keys that must move", not "all progress numbers"; listing a ratio would fail on correct code.
- **There is one irreducible gap.** A counter that was *meant* to be progress but was written with the exclusion passes both guards silently, because nothing in a template context records what question a number was meant to answer. What **is** guaranteed is that a new counter cannot be added silently in the WORKLOAD direction: if it counts mirrors it moves, and an undeclared mover fails, forcing a decision. That is stated in the module docstring too. Do not read more coverage into it than that.

**The 1.3b value sweep is kept alongside it.** `MirrorExclusionSweepTests` still walks every dashboard context and fails on any integer equal to 7 unless the key is declared progress — cheap, already green, and it pins "no unexpected 7 anywhere". It can only ever catch one direction, which is why it is no longer the only guard.

**`MirrorsStayVisibleTests`** pins the visibility direction. **`UserStatusReportTests`** and **`UserStatusRowSumUnmovedTests`** pin the CEO report's row-sum invariant (`not_started + in_progress + completed + blocked == tasks_assigned`) with mirrors present — a WORKLOAD number that 1.6 did not move. **`PmAndOverviewAgreeTests`** is the test the split exists to make possible: the PM dashboard's percentage and the project overview's phase bars, on one project, asserted equal in three states.

---

**R-21 · "Current phase" has ONE implementation, `utils.current_phase()`, and it excludes mirrors.** *(Established by prompt B21, 31 Aug 2026.)*

**"Current" means: the first phase, in `phase_order`, still holding a HUMAN-OWNED task that is not Done.** Mirrors are excluded through `is_human_owned()` — the same helper R-20 uses, not a second predicate. A mirror is nobody's work and nobody can close one, so a phase is not "current" because a mirror in it is open. The question the value answers is *what is this site waiting on*, and no human is waiting on a mirror.
...
**A Python loop over prefetched relations, NOT a queryset, and that is load-bearing.** `phases.filter(...)` ignores the prefetch cache, so the queryset form two of the copies used cost **1–2 queries per project** at every call site — including the two that already prefetched (`admin_project_list`, 50 rows a page, and `dashboard_bd`). The loop form costs **zero** wherever phases and tasks are prefetched, which B21 made true of all four sites by adding the same `Prefetch` clause to `dashboard_pm` and `dashboard_site_engineer`. Measured on six projects: **13 queries before, 0 after.** A new call site prefetches, or it pays 1 + `phase_count` per project.
...
| Vendor invoices | `PaymentRequest` | Live. ~~`pending \| confirmed` ✔~~ **Since O1 (22 Sep): `pending_approval \| approved \| on_hold \| rejected \| confirmed`; stored `pending` renamed to `approved`.** "No edit/cancel by design" ✔ |
| Vendor orders | `VendorOrder`, `VendorOrderSite`, `VendorOrderLine`, `VendorOrderDocument` | **Built by O1** (migration `0091`). RECORDS of orders issued outside PMS; scope is a site set; totals are computed. See §12, 22 Sep |
...
| Notifications | `send_notification()` | Single chokepoint with master switch then per-user preference ✔ — **with two documented bypasses**: `send_raw_email()` skips the master switch, `send_aggregate_email()` skips per-recipient preference ✔. **Every send is a manual call site, with ONE sanctioned exception (O6, 24 Sep 2026):** `signals.payment_transition_notices`, a `post_save` receiver on `StatusTransition` that sends the in-app payment notices for every `payment_request` ledger row (§12, 24 Sep, "Payment notices hang off the LEDGER"). It is suppressed per row by `record_transition(..., notify=False)`, which sets `_notify = False` on the row object before saving it — no column; the receiver reads it off the instance it is handed. **A bulk or corrective script that records payment moves MUST pass `notify=False`**, or every row it writes notifies real approvers and requesters. No other receiver sends, and none should be added without the same switch |
...
|---|---|---|
| A mirror is excluded from overdue and per-user workload counts | 25 call sites, through `human_owned_tasks_q()` / `is_human_owned()` (R-20, WORKLOAD half) | **BUILT — 1.3b, and live since 1.3c copied the snapshot** |
| A mirror is INCLUDED in "how much of this site is done" | 7 call sites, through `site_progress_tasks_q()` (R-20, PROGRESS half) | **BUILT — 1.5 for the phase bar, 1.6 for the PM card and the CEO card pair** |
...
| A mirror does not make its phase "current" | `utils.current_phase()` (R-21) — the one implementation, called by all four screens | **BUILT — prompt B21, 31 Aug 2026.** A fresh OPEX site reads Approvals (Pre-Installation), not Design |
...
because R-20's PROGRESS half **counts** mirrors (1.5, 1.6), those three sit in the denominator
of every OPEX progress fraction and can never reach its numerator — no derivation hook exists,
`_apply_task_status_change()` refuses a human write (B22), and `TaskAdmin` holds `status`
readonly (B9).
...
reaches `awaiting_pm_approval`.** The DEPLOY GATE in `EXECUTION_MODULE_DEFERRED.md` §D was
**SATISFIED on 15 Sep 2026 by prompt 3.1b-3** (`683a0ea`), which is the thing it was waiting
for. `1f2a137` is nevertheless **still unpushed** — `origin/main` was at `6cdb61e` on 16 Sep
2026 — and when it goes it must go **together with or after `683a0ea`**.
...
| **31 Aug** | **`enable_cascade_scheduling` is refused for OPEX too, and it is the door that mattered.** B18 as written named only the *Recalculate dates* control. That control renders in **no template** and never did — its exposure was the view accepting a direct POST. The cascade control **does** render, is reachable by any assigned PM once the Admin flips `SystemSettings.cascade_scheduling_enabled`, is **irreversible by design**, and once on makes `task_set_due_date` refuse every non-PM role owner outright. Eleven of the 22 OPEX tasks are the Site Engineer's. Turning it on would have permanently deleted the only scheduling a tender site has — a worse outcome than one bad recalculation, and undoable by nothing. The system-wide switch defaulting `False` made it latent, not safe. |
| **16 Sep** | **NOTE ON THE 31 Aug `enable_cascade_scheduling` ROW ABOVE. Its reasoning is unchanged; one figure in it was corrected.** It read *"Nine of the 22"*, and nine was **wrong when it was written** — the seed live on 31 Aug (`0075` at `2051f89`) gave the Site Engineer **11 of 22**. That is a transcription error in a dated entry, not stale history, so it was corrected in place on 16 Sep 2026 and the date kept. **The seed then changed on 1 Sep** (`f022b7d`, prompt 1.5): **11 of 23 since**. The Site Engineer's eleven never moved — the two SCM inspections left and `Material Delivery` became four delivery mirrors, so SCM went 3 → 4 and only the denominator changed. Both counted from the two committed revisions of `0075_seed_opex_template_v1.py`. **The decision is unaffected**: a larger Site Engineer share only strengthens the reason it gives. Recorded as `EXECUTION_MODULE_DEFERRED.md` §D36. |
...
| **1 Sep** | **The Payment Milestones card is Residential-only, and `milestone_create` grew the matching guard.** The card was gated on ROLE alone, so an OPEX site rendered "No milestones yet" above a **Create M1 / M2 / M3** button for the exact rows `opex_site_activate` refuses to mint — the screen offering what the backend declines. The whole card is hidden, not just the button, because a card reading "No milestones yet" implies there should be some. **The endpoint was guarded too:** a hidden control is not a disabled one, and `milestone_create` is reachable by POST from a stale tab regardless of what the template renders. The "285 milestone rows already on tender sites" that an earlier reading held back for **do not exist** — spec v1.5 §2a establishes 285 was the A-1.3 audit's *projection*, not a count. **Corrected again 1 Sep by 1.6:** the real number is **12**, counted in production on 01 Sep 2026 — not 285 and not zero. §2a's conclusion ("no such rows exist") is therefore wrong too, and so is one of its premises; nobody yet knows which path created those 12. See **B28**. |
...
| **16 Sep** | **NOTE ON THE 13 Sep ROW ABOVE — its text is unchanged, and was true when it was written.** The condition it names has since been met: prompt 3.1b-3 (`683a0ea`, 15 Sep 2026) built the notifications, and the DEPLOY GATE in `EXECUTION_MODULE_DEFERRED.md` §D is marked SATISFIED. **The hold itself has not been lifted** — `1f2a137` is still unpushed, `origin/main` was at `6cdb61e` on 16 Sep 2026, and it must reach Railway together with or after `683a0ea`. |
| **16 Sep** | **One design-change window for the PM and SCM (session 3.1c-i) — and a site now leaves its procurement group when a change request is ACCEPTED, not when it is raised. This REVERSES Part 4.6's recorded choice.** Part 6 §4 pulled a draft-group site out of its group in the raise's transaction, and Part 4.6 kept it there on purpose so that SCM would not aggregate a BOQ under dispute. That cost SCM a manual re-add after every rejection, and made the "lock refused while a member has a pending request" guard reachable only by re-adding such a site. From 3.1c-i the removal runs inside `design_change_request_accept()`'s transaction; a rejection leaves the site in its group, and `site_group_lock()` — unchanged, it already refused — keeps the group unlockable while the request is pending. The same session: **D-a** the window for a released design is *released and in no locked procurement group* (`permissions.design_change_window_open()`), the same for the PM, coordinators and SCM, and the PM keeps Part 4's pre-release window; **D-b** SCM may raise (`user_may_raise_design_change_as_scm()`), after release only; **D-c** `m_change_request_rate` attributes to the site's `assigned_pm`; **D-d** acceptance re-checks `project_boq_is_group_locked()` under row locks and changes nothing if the BOQ was locked after the raise. Acceptance also clears `released_at` / `released_by` in the status write. No migration, no new reason value. Residual race and other open ends: `EXECUTION_MODULE_DEFERRED.md` §D43. |
| **17 Sep** | **Design change requests notify, in-app AND by email (session 3.1c-ii) — the design module's first sends that name email; the four PM-gate notifications stay in-app only.** Recipients come from three new keys on `design_gate_next_actors()`, whose four gate branches are unchanged. **N-a** a raise tells the site's PM, its active coordinators and every active `is_design_head` holder, not their deputies. **N-b** an acceptance tells the requester, the PM, the coordinators and the allocated designer, plus the SCM owner of a draft procurement group the site left in that transaction (`membership.added_by`, else `group.created_by`); a rejection tells the requester alone, with the Head's reason. **N-c** the actor is never told, nobody is told twice, and the change-request keys drop anyone whose `UserProfile` or `auth.User` is inactive. **N-d** in-app links are relative, from `reverse()`; the email's one absolute link is `request.build_absolute_uri()`, in the HTML part only, because `send_notification()`'s `message` is both the bell text and the email's plain text. The HTML part (`projects/email/design_change_request.html`) is autoescaped. **N-e** each view sends once per recipient after its atomic block, inside `try/except` with `logger.exception`, naming `channels=['in_app', 'email']`. Test settings empty both API keys. No migration, no field, no switch. Open ends: `EXECUTION_MODULE_DEFERRED.md` §D44. |
| **17 Sep** | **Lifting a Design Hold taken from an Arka review status restores that status (the D18 session) — fixed forward; existing orphans are not repaired.** The hold is stored as `survey_returned`. `_status_after_unblock()` stays the one function that decides, with the same two callers (`design_survey_upload`, `design_survey_link_set`). **R1** the from-status on the hold's own `StatusTransition` row is restored only when it is `arka_submitted` or `awaiting_head_arka`; every other from-status is derived as before. **R2** only when the current Arka matches it (`_arka_matches_status()`: both verdicts pending or both approved for `arka_submitted`; QC approved and Head pending for `awaiting_head_arka`); otherwise derived, with a warning. **R3** a hold with no ledger row of its own (the site's latest row is not the hold, so a pre-ledger hold or one written outside the product) is derived, with a warning, and never refused. A ledgered hold from `in_qc`, `artifacts_uploaded` or `awaiting_head_qc` (only possible before §D13) is derived and logged "pre-§D13 hold; needs repair". **R4** no existing site is repaired here (`EXECUTION_MODULE_DEFERRED.md` §D46). After a restore to `arka_submitted` both lift views evaluate `_maybe_advance_to_artifacts_uploaded()` in the same transaction, because CAD and BOQ can land during a hold. No migration, no field. Open ends: `EXECUTION_MODULE_DEFERRED.md` §D18, §D45–§D47. |
| **18 Sep** | **A typed date must fall between 01 Jan 2020 and today + 5 years (the typed-date session).** Production held Task due dates in the years 20 and 26: every parser the views used accepts `0026-09-17`, and a browser date picker lets a two-digit year through. One helper, `forms.check_typed_date()`, returns the date or a message that names the range; it never raises, so `'2026-02-30'` is a message and no longer a 500. It is called at all fourteen entry points that store a typed date. Ten call it directly (`task_set_due_date`, `_apply_task_status_change`, `project_overview`'s milestone edit, `confirm_payment_request`, the three `create_*_issue` views, `create_delivery_challan`, `design_due_date_propose`, `design_due_date_change`). Four reach it through a `clean_<field>` hook on their form (`task_add`, `project_create`, `project_edit`, `project_field_edit`), so the refusal there is an inline field error. **The rule lives in views and forms, not in a model constraint**, because a constraint would refuse an existing bad row on the save that corrects it. Empty input keeps each view's existing behaviour. Malformed input used to be ignored in some views and now refuses the save: the milestone edit used to erase the stored date and report success, the issue views dropped the date, and the challan dropped its expected date. A refused issue keeps its typed title and description in the session and reopens its modal. The design views keep their stricter rule that a date cannot be in the past, which is checked after the range. Date inputs get `min`/`max` from the `typed_date_min`/`typed_date_max` context keys (`context_processors.typed_date_bounds`); the design inputs use `min` = today. `manage.py list_implausible_dates` lists every `DateField` outside the range, read-only. System timestamps, `is_overdue()` and every metric are untouched, and the three production rows were not corrected. Not covered: Zoho, Django admin and query-parameter filters (`EXECUTION_MODULE_DEFERRED.md` G6–G10). |
| **22 Sep** | **`VendorOrder` RECORDS an order issued outside PMS (O1, migration `0091`).** PMS never generates, numbers or approves a PO; `po_number` / `pi_number` are the vendor's or SCM's own. Children: `VendorOrderSite`, `VendorOrderLine` (catalogue and BOQ FKs `SET_NULL`, beside a code/description/unit/category snapshot, the `BOQCorrection` precedent), and `VendorOrderDocument` (bucket + path, no URL; `supabase_storage.vendor_order_document_url()` builds it). No edit, no delete, no soft delete on any of them; every FK into an order is `PROTECT`. Total, paid, balance and invoiced are properties over the children, never columns. `raised_with_unfrozen_quantities` is a fact recorded at raise time, not derived state. |
| **22 Sep** | **An order's scope is a SET OF SITES; a procurement `SiteGroup` is a picker (O1).** `VendorOrderSite` names each site, unique per order. `via_site_group` records which group a site was picked through, for display only, and is `SET_NULL`; nothing reads membership through it. All sites on one order share its `project_type`, enforced in the raise views (O2), not the schema. Residential orders name one or more sites and no group. |
| **22 Sep** | **`PaymentRequest` gains an approval lifecycle (O1, migrations `0091` + `0092`).** Statuses: `pending_approval` "Awaiting approval", `approved` "Approved — awaiting payment", `on_hold`, `rejected`, `confirmed` "Paid". **`pending` is gone:** 0092 renamed every stored `pending` to `approved`, and every `PaymentRequest.PENDING` reference became `APPROVED`, because "with Finance, awaiting payment" is what `approved` means. `raise_payment_request` creates `approved`, so behaviour is unchanged until O4 puts `pending_approval` in front of Finance. A CHECK requires `decision_reason` on `rejected` and `on_hold`. `vendor_order` is nullable only until O2 replaces the raise path. `invoice_number`, the three `invoice_document_*` fields and `boq_item` are legacy, dropped in O6. |
| **22 Sep** | **`UserProfile.is_payment_approver` is a capability flag, not a role (O1, R-15).** Storage only: no predicate and no UI until O4 (R-12). |
| **22 Sep** | **`PaymentRequest` becomes the eighth `StatusTransition` subject (O1, migration `0091`).** `SUBJECT_PAYMENT_REQUEST`, its `SUBJECT_TYPE_CHOICES` entry, and the `_subject_type_registry()` / `_SUBJECT_PROJECT_RESOLVERS` entries exist. **Nothing calls `record_transition()` for it yet**; the call sites land with O4 (approval) and O5 (confirm). §13 lists it as instrumented-pending. |
| **23 Sep** | **`VendorOrderSite` is the REQUIREMENT an order was sized against — not a destination and not an allocation (O2d, migration `0095`).** Naming a site says the quantities were worked out from that site's needs, and nothing more. Material may be delivered to a central store and issued to sites later, in whatever split the work turns out to need, and **none of that is recorded anywhere in this module**: there is no goods-received balance, no issue-to-site and no reservation. Nothing may read these rows as "this much material is owed to this site". **The warehouse process is PLANNED AND NOT OPERATIONAL** — `StockLocation` and its CRUD screens exist (`9a5de0c`, 21 Sep) but nothing issues stock through them. This entry is structure ahead of that process, written now so the sites are not mistaken for an allocation in the meantime. Allocation and stock are out of scope for the order module: `EXECUTION_MODULE_DEFERRED.md` §25. |
| **23 Sep** | **An order may have ZERO sites, and `VendorOrderProgram` records the tenders it was sized against (O2d, migration `0095`).** A stock purchase sized against a tender as a whole names its programs and no site at all. `VendorOrderProgram` is unique per (order, program), derived at raise time from the chosen sites' programs, and — alone among an order's children — **editable and removable by the raiser**, because it records an intention rather than a fact about money. It is **NOT a cost allocation**: no share of the order's total attaches to a program, and nothing may derive one. `user_can_view_vendor_order()` was restated so its site clause is **additive**: a portfolio role (CEO, Admin, System Admin, Finance, SCM) decides on its own and the walk over sites can only ADD readers, so a site-less order stays readable by the five and unreadable by an unentitled PM. The predicate's behaviour did not change — the early portfolio return already made it additive; the words did. Residential is unmoved: `vendor_order_create` still writes exactly one site, and its twenty tests pass unchanged. |
| **23 Sep** | **`PaymentRequest.project` is a DISPLAY ANCHOR, not scope, and is nullable again (O2d, migration `0095` relaxes `0093`'s NOT NULL).** It is set to the order's lowest `project_id` when the order has sites, and NULL when it has none — `order_views._order_project()` is the one rule that computes it, for the payment, its feed line and its ledger row alike (`ActivityLog.project` and `StatusTransition.project` are both already nullable). **Money and scope always come from `vendor_order`**; `VendorOrder.scope_label` is the one phrase a screen shows when it wants to say what a payment was for (the site's `project_id` for one site, "N sites" for several, the tender names for a site-less order, `Order #<pk>` otherwise), and `PaymentRequest.scope_label` delegates to it so the two can never disagree. **Every reader that filters on `project` — the Finance and CEO dashboards, `project_overview` card 4b, `payment_request_detail`, `confirm_payment_request`, `my_documents` — silently EXCLUDES a NULL-anchor payment rather than crashing on it**, asserted by `tests_vendor_order_scope.NullAnchorReaderTests`. Making such a payment visible is O5's and O6's work; O2d was barred from those screens. |
| **24 Sep** | **A raised payment waits for an approver; `pending_approval` is now the default and what every raise path writes (O4, migration `0096`).** `vendor_order_create`, `vendor_order_create_group` and `vendor_order_add_payment` all create `PENDING_APPROVAL` and record that status, and `PaymentRequest.status`'s model default moved from `approved` to `pending_approval` so a row made in a shell, a fixture or a seed lands in front of the gate rather than past it. **No data migration and no backfill**: this database held zero `PaymentRequest` rows, and a row raised before O4 would stay `approved` deliberately — it was approved under the rule then in force, and back-dating it into a gate that did not exist would invent a decision nobody made. `committed_total()` is untouched and still counts every status but `REJECTED`, so an order's committed and available figures did not move. **Every reader that filters `status=APPROVED` — `dashboard_finance`, `_get_ceo_dashboard_context`, `confirm_payment_request`, `project_overview` card 4b's badge — now simply shows fewer rows, none of them crashes, and none was changed here: the Finance queue and the mark-paid path are O5.** |
| **24 Sep** | **Approve, hold and reject are three different acts, and reject is reachable ONLY from hold (O4).** `payment_approve` moves `pending_approval` or `on_hold` to `approved` and stamps `approved_by`/`approved_at`; its remark is the one OPTIONAL text in the module. `payment_hold` moves `pending_approval` **or `approved`** to `on_hold` with a MANDATORY reason — an approver who learns something after approving must be able to stop the request, because money moves at confirmation and not at approval. `payment_reject` is final, needs a MANDATORY reason, and refuses every status but `on_hold`: **SCM must have had the chance to answer before a request is killed**, and that is one extra click for the approver and the whole protection for procurement. `payment_hold_respond` is SCM's side — it writes the answer and returns the request to `pending_approval`, **clearing `approved_by`/`approved_at`** if the hold was taken on an approved request, because an approval that has been reopened is not an approval. All four are POST-only, lock the row with `select_for_update()` and **re-check their predicate inside that lock**; the check outside it draws the button, the check inside it decides. A status that no longer permits the action is a message and a redirect naming the status the request is actually in — never a 500 and never a silent no-op — while a person who was never entitled gets 403. |
| **24 Sep** | **One hold, one response, and a second hold is a second ROW (O4, `PaymentRequestHold`, migration `0096`).** A hold records its reason, who took it and when; SCM's answer records the text, the responder and the time, written once and together (a `CHECK` requires all three or none). **Nothing is edited after it is written** and a request may be held any number of times — `uniq_open_hold_per_payment_request` is a PARTIAL unique index on `responded_at IS NULL`, so exactly one hold is open at a time and every earlier reason and answer survives. The precedent is `DueDateCommitment`: a two-sided exchange that is never edited in place, with a partial index keeping one of them live. **`payment_request_refusal_needs_reason` narrowed from `['rejected', 'on_hold']` to `['rejected']`** as part of this — not a relaxation, because the hold's reason is now required on the row that carries it, and leaving `on_hold` in the old constraint would have demanded the same text in two places that could then disagree. An approval or a rejection LEAVES an open hold open: it was overtaken or overruled, not answered, and marking it answered would put words in SCM's mouth. |
| **24 Sep** | **`UserProfile.is_payment_approver` is consumed, and nobody approves their own request (O4).** The flag gains its first callers — `permissions.user_is_payment_approver()` reads the flag and nothing else, and `user_can_approve_payment()`, `user_can_hold_payment()`, `user_can_reject_payment()` add the per-request terms — the same split as `user_is_design_qc()` beside `user_can_qc_gate_design()`. **All three refuse the requester**, whatever flags they hold: this is settled decision 2 (`design_views._other_gate_actor_conflict`) applied to money, and it is PER REQUEST, not per user — an SCM lead holding the flag approves everybody else's requests and is refused only their own, and the order page tells them why rather than showing an empty cell. `user_can_respond_to_hold()` is the fifth and is ROLE-based (`VENDOR_ORDER_RAISE_ROLES`), not flag-based, and is deliberately **not** narrowed to the person who raised the payment: procurement answers as a department. The flag is editable on the Admin Panel's user edit screen ("Payment approver"); the System Admin's own edit screen does not carry it, exactly as it does not carry `is_qaqc` or `is_warehouse_keeper` — see `EXECUTION_MODULE_DEFERRED.md` §O4-1. |
| **24 Sep** | **Finance works vendor payments from one queue, with a tab per project type (O5, `payment_views.payment_queue` at `/payments/`, no migration).** Three tabs — Residential, RESCO, CAPEX — and **no combined tab**: each tab's label carries its own "N to approve · N to pay", so one tab's backlog is visible from the others without a fourth list to hide it in. **The tab is `vendor_order.project_type`, NEVER `pr.project`**, and nothing on the page filters on a project's status or joins through `project` for scope: `project` is the nullable display anchor (O2d), and reading scope through it drops every site-less payment — the defect `EXECUTION_MODULE_DEFERRED.md` §25 recorded against the Finance dashboard. A payment on a central purchase and a payment on a Draft OPEX site are listed like any other. Within a tab: tiles (awaiting approval, approved to pay, on hold — count and ₹ each — and orders with invoice awaited), status chips, a search over vendor, PO, PI, tender and site code, and rows newest first, 50 to a page. **Portfolio-wide, per D-4**: `permissions.user_can_view_payment_queue()` admits Finance, CEO, Admin, System Admin and any payment-approver flag holder; SCM, PM and Design get 403. The nav's "Payments" entry is gated by the same predicate (`context_processors.payment_queue_nav`), and the Finance dashboard's payment tile links to the queue. The page costs the same number of queries for 1 row as for 50 (`tests_payment_queue.QueryCountTests`). |
| **24 Sep** | **The queue reuses the O4 action views unchanged, and returns to itself (O5).** Approve, hold and reject on the queue post to `payment_queue_action`, which calls `payment_approve` / `payment_hold` / `payment_reject` with the same request and, when a safe `next` names the queue, swaps the redirect's destination for it. The O4 views do not read `next` and were not edited: their predicate checks, locks, ledger rows, messages and 403s are exactly O4's, and a 403 passes through as they gave it. A `next` that is off-host, or is not the queue, is ignored, so the swap cannot become an open redirect. |
| **24 Sep** | **The approver and the payer are different people on one request, and a payment reference is mandatory (O5).** `permissions.user_can_mark_paid()` is Finance AND `approved` AND not `approved_by` AND not `requested_by` — the QC/Head precedent (settled decision 2) applied to the last step, per request and not per user; with five Finance users it never deadlocks. **`payments.mark_payment_paid()` is the ONE writer of `approved -> confirmed`**: it locks the row, re-checks the predicate inside the lock, refuses a blank reference, a date in the future and a date before the request was raised, writes the four fields and records the transition with the reference as its remark. The queue's "Mark paid" and the project page's `confirm_payment_request` both call it and nothing else of their own, so they refuse the same things in the same words. `confirm_payment_request` no longer filters on status in its lookup, so a request held after the page loaded is a message naming its status rather than a 404. A reference was optional before O5, and rows confirmed before O5 legitimately carry none. |
| **24 Sep** | **`invoice_paid` is removed, not disabled, until O7 (O5).** `confirm_payment_request` sent it on in-app, WhatsApp and email to every SCM, the project's managers and every CEO. The call is gone. **Four in-app notices replace it**, sent by `payments.send_payment_notices()`: a hold tells the requester; SCM's answer tells every active approver except the requester; a raise tells every active approver except the requester; mark-paid tells the requester. Approve and reject tell nobody. Every notice is `channels=['in_app']` with no template, links to the order, and is never sent to the person who acted. O7 widens the channels with templates written for the order module. |
| **24 Sep** | **Payment notices hang off the LEDGER, not the views (O5).** A `post_save` receiver on `StatusTransition` (`signals.payment_transition_notices`) fires for `payment_request` rows only and defers `send_payment_notices()` with `transaction.on_commit()`. That is the one place every payment move is recorded (§13), so the hold and the answer are covered without a line changed in the four O4 views; a move that rolls back — a refusal under the lock, a lost race — tells nobody; and an idempotent replay of `record_transition()` saves nothing and so notifies nobody twice. The consequence to know: **any new write site that records a `payment_request` transition notifies automatically** — decide its notice in `payments._notice()`, not at the call site. |
| **24 Sep** | **Every payment figure comes from ONE count, read through the order (O6, `payments.payment_counts()`).** It returns, for each of the five statuses, a count, the sum of `effective_amount` and the sum of `amount`, zero-filled, in one grouped query over `vendor_order__project_type` — never through `PaymentRequest.project` and never filtered on a project's status. It takes a list of project types (`None` = all). The payments queue calls it once per tab with a one-item list; `dashboard_finance`'s two payment tiles and `_get_ceo_dashboard_context`'s two finance tiles call it with the display context's types (`views._context_types()`, which reads `CONTEXT_PROJECT_TYPES` exactly as `_context_filter()` does). **The three figures therefore cannot disagree**, and a site-less payment, a payment on a Draft OPEX site and a partially approved one are counted on all three (`tests_payment_readers_o6.ThreeWayAgreementTests`). The dashboards' tiles are the approved-to-pay count and value; the CEO context's `active_filter`, a project filter used only by those two tiles, was removed. Nothing else in either dashboard changed. |
| **24 Sep** | **One rule for showing a payment's amount, one partial (O6, `projects/partials/payment_amount.html`).** One figure when what was requested is what counts; "₹X requested · ₹Y approved" while an approval for less stands (`is_partially_approved`); and "₹X requested · ₹Y previously approved" on a request back awaiting approval or on hold with an approval on record — the O4c ceiling, which survives a hold. The queue, the order page, card 4b, My Documents and the Finance dashboard's rows all include it; the two dashboard value tiles apply the same rule to their sums. A rejected request shows its requested amount: it counts nothing. |
| **24 Sep** | **`project_overview` card 4b lists ORDERS, not payments, and pays nothing (O6).** Every order with this site in `VendorOrderSite`, newest first — so a group order whose display anchor is a different site is listed on every site it was sized against — each with its vendor, PO / PI, total and paid (`order_views._order_money()`, two prefetches, flat in the number of orders), its payments with the model's five labels, and a link to the order; the card links the site's order list (`user_can_view_project_vendor_orders`) and, for anyone passing `user_can_view_payment_queue`, the payments queue. **The Finance confirm button, its modal and its JS are gone: Finance pays from the queue only — one path to pay, not two.** The card keeps its role gate (Finance, PM, SCM, Admin). |
| **24 Sep** | **The payment detail page is a redirect, and My Documents reads through the order (O6).** `payment_request_detail` keeps its URL so old links resolve, looks the payment up by its pk ALONE — the `project_id` in the URL is not checked, because a site-less payment has none — and redirects to `vendor_order_detail#payment-<pk>`, whose own gate (`user_can_view_vendor_order`) now does the refusing: a PM with no site on the order gets 403 there, where the old page answered 404. Its template is deleted. My Documents' section E filters on the requester alone, so a site-less payment is listed; each row names its order (PO, else PI, else `Order #<pk>`), its scope (`scope_label`), the five-state label, and links to its order's row. |
| **24 Sep** | **The stand-alone raise and confirm views are deleted, and `PaymentRequest` loses five legacy columns (O6, migration `0101`).** `raise_payment_request` (a redirect and a 410 since O2) and `confirm_payment_request` (a door onto `mark_payment_paid()` since O5) and their URLs are gone; both old paths now 404. `mark_payment_paid()`'s one caller is the queue's `payment_mark_paid`, which is now typed-date entry point #8 (the 18 Sep row named the confirm view). **0101 drops `boq_item`, `invoice_number` and the three `invoice_document_*` columns, and first REFUSES** — naming the rows — if any holds a value: any text but `''`, or a non-NULL `boq_item`. It never discards; the check is plain SQL over a table name so a test can run it against a table that still has the columns. The three raise flows stopped writing the four blank strings, and nothing else in them changed. `teardown_order_demo`'s guard lost its `PaymentRequest.boq_item` arm; `VendorOrderLine.boq_item` was and is still checked, so the guard is not weaker. |
| **24 Sep** | **Only Finance and CEO may hold the payment-approver flag (O6, `permissions.PAYMENT_APPROVER_ROLES`).** `AdminUserEditForm.clean()` refuses `is_payment_approver` for any other role, as a field error, against the role being SAVED — so re-roling a flag holder is refused until the box is cleared, and the flag cannot outlive the role that justified it. A flag holder is therefore always a portfolio reader of the queue and of every order, which closes `EXECUTION_MODULE_DEFERRED.md` §O5-4 at its source. No data change: existing flag holders are not touched (§28 records what to check before deploy). |
| **24 Sep** | **`record_transition()` gains `notify=True`; `notify=False` records the row without the notices (O6).** Stored on the Python object as `_notify`, NOT in a column — `post_save` hands `signals.payment_transition_notices` the very instance `record_transition()` built, and nothing reads the flag after that. A row saved any other way has no `_notify` and notifies as before. This is the one sanctioned exception to "every send is a manual call site" (§5, Notifications): **a bulk or corrective script that records payment moves must pass `notify=False`**. No test or script in the tree records a payment transition directly for setup today, so no existing call site needed the flag; the product's six write sites keep the default. |
| **24 Sep** | **Two labels and one tab (O6).** The SCM dashboard's tender row reads "PO / PI records (n)" rather than "Payment requests (n)" — it counts orders, not requests. The payments queue draws its CAPEX tab only while at least one CAPEX order exists; asking for the missing tab lands on the first. CAPEX has no payment-request raise path and is not operational (`EXECUTION_MODULE_DEFERRED.md` §28). |
| **25 Sep** | **A design change request carries who raised it and where it stands on an SCM route that does not exist yet (session B1, migration `0102`, schema only).** Decided by Zuber, 25 Sep 2026: the PM may forward or reject an SCM-raised request, with a mandatory note (D-1, D-3). The assigned PM and active coordinators may triage (D-2). Any SCM user may withdraw it while it is with the PM, with a note (D-4). The PM-raised path is unchanged (D-6). The Head gains a third outcome, "corrected in the BOQ" (D-7). Requests pending at deploy stay with the Head (D-8). The stages are new **verdict values**, not a stage field (D-9): `with_pm`, `pm_rejected`, `withdrawn`, `corrected`. `verdict` widens to 20 characters. **`origin`** (`pm` / `scm`) has **no model default**, and `cr_origin_valid` refuses the `''` a forgotten one would store. The raise view sets it from `user_can_manage_project()`, the predicate its activity line already used, so column and log agree. The migration backfilled existing rows from that log line: production had zero rows, and locally pk 4 became `scm`. Five CHECKs: a PM decision (`pm_decided_at`, not the verdict, since a forwarded request is `pending` like a PM-raised one) needs `pm_note`; `withdrawn` and `corrected` each need their person, time and note; `with_pm`, `pm_rejected` and `withdrawn` are SCM-only; `corrected` is open to both origins. `uniq_pending_change_request_per_attempt` keeps its name and covers `with_pm` too. `pm_decided_by`, `withdrawn_by` and `corrected_by` are PROTECT. The evidence for `corrected` is an M2M to `BOQCorrection` (`related_name='+'`), so neither that model nor `boq_correct` changed, and B2's view must require at least one link. Admin shows every new field read-only, and `origin` only on the add form. `opened_reason`'s `pm_change_request` is labelled "Change request"; the stored value is unchanged. **Nothing writes a new verdict until B2; B3 owns the wording and the metrics** (`EXECUTION_MODULE_DEFERRED.md` §D49). |
| **25 Sep** | **An SCM-raised design change request goes to the site's PM before the Design Head, and the Head can record a request as corrected in the BOQ (session B2a, no migration).** Decided by Zuber, 25 Sep 2026 (D-1..D-7 as in the B1 row) and at B2a sign-off. The raise view evaluates `user_can_manage_project()` once for `origin`, the log line and the routing: a PM- or coordinator-raised request is `pending` (D-6); an SCM-raised one is `with_pm` — **unless the site has no assigned PM and no active coordinator (Q5)**, when it is `pending`, and the raise message, its activity line (action_code `design_change_requested_no_pm`) and the request row on `change_request.html` all say it went straight to the Head. Who may triage is `user_can_triage_scm_change_request()` and whether anybody can is `scm_change_request_has_triager()`; both read `project_managers()` (assigned PM + ACTIVE coordinators), so they cannot disagree. Four new endpoints, each re-reading the verdict under a row lock and each with a mandatory note: **forward** (`with_pm`→`pending`, the `pm_decided_*` fields and `pm_note`; re-checks `design_change_window_open()` under request→group→assignment locks), **PM reject** (`with_pm`→`pm_rejected`), **withdraw** (any SCM user, `with_pm`→`withdrawn`), and the Head's **corrected** (`pending`→`corrected`, either origin; links every BOQCorrection on the site's BOQ made at or after `requested_at` and not already cited, and refuses with none; opens no attempt, keeps `released_*` and the group membership). New action_codes: `design_change_request_forwarded`, `_pm_rejected`, `_withdrawn`, `_corrected`, and `design_change_requested_no_pm`. **A request with the PM is outstanding (A1):** `_open_change_requests()` (verdict in `with_pm`/`pending`) feeds the QC suspension, the raise pre-check and `pending_change_requests_for()` (the group-lock blocker); `_pending_change_requests()`, the head-sites prefetch and `design_metrics`' `pending_crs` stay pending-only, because the request is not with the Head. Six refusal/raise messages gained a with-PM or SCM branch; the PM path's are byte-identical and a test says so. `change_request.html` and `_attempt_history.html` render all seven verdicts in their own branches; no value falls into "Awaiting the Design Head". **No notification changed** — B2b reroutes them. **HARD DEPENDENCY: B2b must add the "corrected" button to the QC review banner**; until then the endpoint is POST-only, and the browser walk happens after B2b. B1, B2a and B2b deploy together (`EXECUTION_MODULE_DEFERRED.md` §D50). |
| **25 Sep** | **The SCM change-request route gets its screens and notifications (session B2b, no migration).** Decided by Zuber at the B2b sign-off, 25 Sep 2026 (D-10, D-11, D-12, A7 part 1). **Who may triage** is now one list, `permissions.scm_change_request_triagers()`: the assigned PM and every coordinator, each only when BOTH the UserProfile and the auth.User are active. `user_can_triage_scm_change_request()`, `scm_change_request_has_triager()` (so the Q5 fallback), the PM queue's section and the with-PM raise audience all read it, so an inactive assigned PM no longer counts as a forwarder. `NO_PM_FALLBACK_TEXT` reads "this site has no active PM and no active coordinator". `project_managers()` is unchanged. **Screens:** The Head's "Record as corrected in the BOQ" form sits on `qc_review.html`'s change-request banner, beside Accept and Reject. It is behind the same `has_head_authority` flag, takes a mandatory note, and shows how many uncited BOQ corrections it would cite. It stays pressable at zero, and the endpoint says why it refuses. `design_pm_approval_queue` gains "SCM change requests waiting for you". These are the `with_pm` requests on the viewer's triage sites, oldest first, with days with the PM from `requested_at`, and each links to the per-site page, where the forms stay. There is still no role gate: anyone else gets 200 and no rows. The nav entry is renamed "Design & BOQ approvals", in `base.html` only; no other shell has it. `head_sites` Pending at reads "PM (change request)" for a `with_pm` request, through `design_pending_at(change_request_with_pm=)`. A pending request's label is unchanged. **Ageing (D-11):** the Head's queue, its attention band and the timestamp under the tender dashboard's badge count from `pm_decided_at` for a forwarded request and from `requested_at` otherwise (`design_metrics._head_clock_start`). No metric definition changed. **Notifications (D-12):** every send is in-app and email, one call per recipient, after the atomic block, inside a try, with the actor excluded and nobody told twice. An SCM raise to the PM reaches the PM and the active coordinators, not the Heads. A Q5 raise reaches the Heads and says why the PM stage was skipped. A PM raise is unchanged. A forward reaches every Design Head and the requester. A PM reject reaches the requester. A withdrawal reaches the PM and the coordinators. A Head reject of an SCM-origin request also reaches the PM and the coordinators. A Head accept is unchanged. "Corrected" reaches the requester, the PM and the coordinators, with up to three "CODE before → after" corrections, then "and N more". Six new `design_gate_next_actors()` keys cover these. Its six earlier branches and the shared tail are AST-identical to B2a, and the four gate notifications are unchanged. The Head's pull of a stranded `with_pm` request and the corrected button on the tender dashboard are deferred (`EXECUTION_MODULE_DEFERRED.md` §D51). |
| **26 Sep** | **Change requests are findable from the two screens named for the work (session B3a, no migration).** Decided by Zuber at the B3a sign-off, 26 Sep 2026 (D-13..D-16, sign-offs 1–6). **The PM queue** (`design_pm_approval_queue`) lists every change request on the viewer's sites, of any verdict and either origin, in one query with the same scope as B2b's section (`manageable_projects_q()` + active profile and user, `.distinct()`): `with_pm` rows stay in "waiting for you", oldest first; the rest are "what happened", newest act first, the State column naming the act and Last acted giving who and when (`_change_request_last_act()`, one branch per verdict; never acted on after the raise falls back to the raise, labelled so). **The design review queue** (`design_qc_queue`) opens with "Change requests waiting for the Design Head" for Head authority ONLY — as `attention_list(own_only=True)` keeps triage off Design QC's dashboard — built by `design_metrics.head_change_request_list()`: every `pending` request across tenders, one query, oldest by `_head_clock_start()`, capped at 25 with the remainder counted on the page and never dropped; no forms, the site links to the `qc_review` banner. **The tender dashboard** queue gains the "Record as corrected in the BOQ" form beside Accept and Reject, gated on `has_head_authority`, with its own mandatory note, a `next` back to the dashboard and the banner's count of uncited corrections, batched by `_uncited_correction_counts()` (one query, none when empty). No permission, transition or notification changed; every transition view is byte-identical. Labels left wrong for B3b are listed in `EXECUTION_MODULE_DEFERRED.md` §D52. |
| **26 Sep** | **Change requests are named by who raised them, and two figures are corrected (session B3b, no migration).** Decided by Zuber at the B3b sign-off, 26 Sep 2026 (D-17..D-22, a–d). **Names (D-18):** a request SCM raised is a *BOQ change request*; one a PM or coordinator raised is a *design change request*; a screen or message about both kinds says *change request*. Text about ONE request names its kind from the stored `origin` through `design_metrics.change_request_noun()` — flashes, activity lines, notification subjects and bodies, the attention band — and no query is added. No screen or message says a `with_pm` request waits for the Design Head. Stored values never change (D-17): `origin`, verdicts, `opened_reason` (`pm_change_request`, label "Change request" since B1, D-19), `CHANGE_REQUEST_REMOVAL_REASON` ('PM change request'), action codes, notification template labels and URL names are as they were. **`m_cr_rejection_rate` (D-20):** Head rejections over requests that REACHED the Head — pending, accepted, rejected, corrected (`CR_REACHED_HEAD_VERDICTS`); `with_pm`, `pm_rejected` and `withdrawn` are left out and counted beside the figure. **`m_cr_by_stage` (D-21):** a fourth bucket, "Raised after release", checked first: `origin == 'scm'` (SCM may raise only on a released site), or `head_verdict == 'passed'` and `requested_at >= head_reviewed_at` (after the Head's pass, release is the only stage in either raise window). A PM's request on an attempt released before Part 9 still reads "with the Head"; production was not checked. **quality_analytics (D-22):** the change request rate is counted against the site's PM, whoever raised it, as the code and the catalogue already said. Every transition view and every `send_notification` call node is proved by a masked AST (every string literal and f-string masked) — only `_not_pending_refusal`, `m_cr_rejection_rate` and `m_cr_by_stage` differ with the mask on. Open ends are `EXECUTION_MODULE_DEFERRED.md` §D53. |
...
| `payment_request` | `PaymentRequest` — `pending_approval \| approved \| on_hold \| rejected \| confirmed` | **INSTRUMENTED SINCE O4 (24 Sep 2026), COMPLETE SINCE O5 (24 Sep 2026) — the six write sites below, and no others.** ① the raise, three paths that agree (`vendor_order_create`, `vendor_order_create_group`, `vendor_order_add_payment`), each recording `'' -> pending_approval`; ② `payment_approve` (`pending_approval | on_hold -> approved`); ③ `payment_hold` (`pending_approval | approved -> on_hold`); ④ `payment_reject` (`on_hold -> rejected`, and from nothing else); ⑤ `payment_hold_respond` (`on_hold -> pending_approval`); ⑥ `payments.mark_payment_paid()` (`approved -> confirmed`, remark = the payment reference), the one writer behind the queue's `payment_mark_paid` (and, until O6 deleted it, the project page's `confirm_payment_request`). Each writes inside the same `transaction.atomic()` and the same `select_for_update()` as the status change, and carries the actor and the reason — mandatory on ③, ④, ⑤ and ⑥, optional on ② alone. Registered by O1 (migration `0091`): the constant, the `SUBJECT_TYPE_CHOICES` entry and the registry/resolver entries. ~~**THE SIXTH TRANSITION IS STILL MISSING**: `confirm_payment_request` writes `approved -> confirmed` with NO ledger row, and that call site is O5's.~~ **Closed by O5: confirming now records `approved -> confirmed`.** A missing row on any status now means the change did not happen — EXCEPT a payment confirmed before O5, which carries no row and never will. Every row written here also triggers the in-app notices (§12, 24 Sep, "Payment notices hang off the LEDGER"). `PaymentRequestHold` is a separate model with no status of its own and is not a subject type — see the second table |
...
| `PaymentRequestHold` | none — it has no status column | **Added by O4** (migration `0096`). A hold is born open and becomes answered by gaining `response`/`responded_by`/`responded_at`, which is a fact about the row and not a status vocabulary. The MOVE it accompanies is recorded against `payment_request` (first table), so instrumenting it too would double-count one event. |
...
add their own row here when reading; that is now done, so these two tables need no mental
patching. **They are not, however, every status-bearing model.** Sixteen models carry a field
named `status`; thirteen are in the two tables above, and **three are in neither — `Program`,
`DesignSubmission` and `NotificationLog`.** None of the three is a `subject_type`, so a missing
row for any of them means nothing, and none has been assessed for instrumentation. Counted
16 Sep 2026.)*
