...
| `payment_request` | `PaymentRequest` — `pending_approval \| approved \| on_hold \| rejected \| confirmed` | **INSTRUMENTED SINCE O4 (24 Sep 2026) — the five write sites below, and no others.** ① the raise, three paths that agree (`vendor_order_create`, `vendor_order_create_group`, `vendor_order_add_payment`), each recording `'' -> pending_approval`; ② `payment_approve` (`pending_approval | on_hold -> approved`); ③ `payment_hold` (`pending_approval | approved -> on_hold`); ④ `payment_reject` (`on_hold -> rejected`, and from nothing else); ⑤ `payment_hold_respond` (`on_hold -> pending_approval`). Each writes inside the same `transaction.atomic()` and the same `select_for_update()` as the status change, and carries the actor and the reason — mandatory on ③, ④ and ⑤, optional on ② alone. Registered by O1 (migration `0091`): the constant, the `SUBJECT_TYPE_CHOICES` entry and the registry/resolver entries. **THE SIXTH TRANSITION IS STILL MISSING**: `confirm_payment_request` writes `approved -> confirmed` with NO ledger row, and that call site is O5's. Until it lands, a missing row on a `confirmed` payment means "not instrumented", and a missing row on any other status means the change did not happen. `PaymentRequestHold` is a separate model with no status of its own and is not a subject type — see the second table |
