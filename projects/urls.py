...
urlpatterns = [
    ...
    path('projects/<str:project_id>/activate/',                views.project_activate,           name='project_activate'),           # PM only, POST only — RESIDENTIAL: creates phases/tasks/milestones, requires a designer
    ...
    path('projects/<str:project_id>/phases/<int:phase_id>/reorder-tasks/',            views.phase_tasks_reorder,             name='phase_tasks_reorder'),              # task_add's gate + can_reorder_phase; POST only
    ...
    # SESSION E1 — the OPEX BOQ as a spreadsheet. A READ, so unlike boq/entry/ it takes
